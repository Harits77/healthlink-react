...
function Login(){
    ...
    const handleSubmit = (e) => {
        ...
        axios
            ...
            .then((result) => {
                console.log(result.data);  // This should now log an object with message and userId
                if (result.data.message === "Login Successfull") {
                    localStorage.setItem("userId", result.data.userId);  // Save the userId to localStorage
                    console.log("Navigating to dashboard");
                    navigate("/dash");  // Now this should work
                } else {
                    alert(result.data);  // In case of an error message
                }
            })
            
            
            .catch((err) => {
                console.error(err); // Debugging: Log error
    ...
    return (
        ...
        <div className="bg-white p-6 rounded w-1/4">
            ...
            <p className="mt-4 ml-3 font-semibold text-gray-600 mb-4">Does not have an account? <Link to="/register" className="ml-3 font-semibold  text-blue-800">Sign Up</Link></p>
